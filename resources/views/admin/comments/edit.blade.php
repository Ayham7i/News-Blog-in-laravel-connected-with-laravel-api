...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('admin.comments.update', $comment['id']) }}" method="POST" class="bg-gray-800 p-8 rounded-lg shadow-lg">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea name="content" id="content" rows="4" class="w-full px-4 py-2 bg-gray-900 text-white rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" required>{{ $comment['content'] }}</textarea>
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="article_id" id="article_id" class="w-full px-4 py-2 bg-gray-900 text-white rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" required>
                ...
                <option value="{{ $article['id'] }}" {{ $comment['article']['id'] == $article['id'] ? 'selected' : '' }}>
                    {{ $article['title'] }}
                </option>
                @endforeach
            </select>
